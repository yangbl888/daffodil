...
/**
 * This is the model class for table "rbac_join_role_user".
 *
 * The followings are the available columns in table 'rbac_join_role_user':
 * @property string $Id
 ...
 */
class RbacJoinRoleUser extends CActiveRecord
{
	/**
	 * Returns the static model of the specified AR class.
	 * @param string $className active record class name.
	 * @return RbacJoinRoleUser the static model class
	 */
	...
	public function rules()
	{
		...
		return array(
			array('Id, Code, Name, CreateOn', 'required'),
			array('AllowEdit, AllowDel, SortCode, DeletionFlag, EnabledFlag', 'numerical', 'integerOnly'=>true),
			array('Id, UserId, RoleId, Code, CreateBy, CreateUserId', 'length', 'max'=>40),
			array('Name', 'length', 'max'=>80),
			array('Description', 'length', 'max'=>200),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('Id, UserId, RoleId, Code, Name, AllowEdit, AllowDel, SortCode, DeletionFlag, EnabledFlag, CreateOn, CreateBy, CreateUserId, Description', 'safe', 'on'=>'search'),
		);
	}
