...
class PubPersonController extends Controller
{
	/**
...
/**
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['PubPerson']))
		{
			$model->attributes=$_POST['PubPerson'];
			if($model->save()){
	...
	public function actionUpdate()
	{
		...
		if(isset($_POST['PubPerson']))
		{
			$model->attributes=$_POST['PubPerson'];
			if($model->save())
                             DwzHelper::success('更新完成！','art_manager');//要自动刷新就把后面的mArticle改成你的navTablId(就是打开navTab的链接中的rel)不用刷新可直接调用$this->dwz();即可
			else
                             DwzHelper::error($model);
		}
	...
	public function actionDelete()
	{
		...
		// 删除操作只允许POST请求操作
                if($this->loadModel()->delete()){
                  // DwzHelper::success('删除成功！','art_manager');
                    
                   DwzHelper::success('删除成功', 'art_manager','forward',''); 
            }
		}
		else
	...
	{
		$dataProvider=new CActiveDataProvider('PubPerson');
		$this->renderPartial('demo',array(
	...
	{
		$model=new PubPerson('search');
		$model->unsetAttributes();  // 清除默认值
		if(isset($_GET['PubPerson']))
			$model->attributes=$_GET['PubPerson'];
	...
	public function loadModel()
	{
		...
		{
			if(isset($_GET['id']))
				$this->_model=PubPerson::model()->findbyPk($_GET['id']);
			if($this->_model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='pub-person-form')
		{
